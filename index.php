...
/*        //*******************************************************************************
        // compte toutes les recettes suivant la lettre et la catégorie et retourne le nombre de pages a afficher
        //*******************************************************************************
        elseif ($_GET['action'] == 'countNumberRecipeAlphaAndCateg') 
        {
            if (isset($_SESSION['idUser']) && ($_SESSION['idUser'] > 0)  &&  (isset($_POST['idcategory'])) && (isset($_POST['alphaRecipe'])))   // test id pour les utilisateur
            {
                 countNumberRecipeAlphaAndCateg($_POST['idcategory'], $_POST['alphaRecipe']); 
            }
            else
            {
                throw new Exception('Aucun compte n\'a été trouvé pour compter le nombre de recette de la catégorie');
...
/*        //*******************************************************************************
        // écrire une recette
        //*******************************************************************************
        elseif ($_GET['action'] == 'writeRecipes') // écrire une recette
        {
            if (isset($_SESSION['idUser']) && $_SESSION['idUser'] > 0) // test id pour les utilisateur
            {
                writeRecipes(); 
            ...
            {
                throw new Exception('Aucun compte n\'a été trouvé, Vous devez vous reconnecter');
        ...
        //*******************************************************************************
        elseif ($_GET['action'] == 'modifRecipes') //modifier une recette
        {
            if (isset($_SESSION['idUser']) && $_SESSION['idUser'] > 0) // test id pour les utilisateur
            {
                if (isset($_POST['idRecette']) && $_POST['idRecette'] > 0) // test id de la recette a modifier
                {
                    ...
                    {
                        throw new Exception('Aucune recette n\'est trouvée pour la modifier');
                    }
            }
            else
            {
                throw new Exception('Aucun compte n\'a été trouvé pour modifier la recette');
        ...
        //*******************************************************************************
        // actualize une recette
        //*******************************************************************************
/*        elseif ($_GET['action'] == 'actualizeRecipes')
        {
            if (isset($_SESSION['idUser']) && $_SESSION['idUser'] > 0) // test id pour les utilisateur
            {
                if (isset($_POST['idRecette']) && $_POST['idRecette'] > 0 && isset($_POST['colBdd']) && (isset($_POST['value'])))
                {
                    actualizeRecipes($_POST['idRecette'], $_POST['colBdd'], $_POST['value']); 
                }
                 else
                {
                    throw new Exception('Aucune recette n\'est trouvée pour la modifier');
