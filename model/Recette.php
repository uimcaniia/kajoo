...
class Recette extends RecetteManager
{
    ...
    // nbr de lettre à conserver pour un extrait d'épisode
    ...
    // recette privée (1) ou public(0) avec les copain du compte
    private $_id_category;

    // id de la catégorie de la recette
    private $_alpha;

    // lettre de rangement alphabetique pour trier : correspont à la première lettre du titre de la recette
    private $_love;

    // indicateur de la recette ( aime )
    private $_price;

    // définit le cout de la recette
    private $_easy;

    // indicateur de la facilité
    private $_id_user;
            ...
            for ($i = 0; $i < count($aIngredientRecette); $i ++) {
                foreach ($aIngredientRecette[$i] as $key => $value) {
                    if ($key == 'id_ingredient') {
                        ...
                        if (file_exists('public/img_ing/'.$aIngredientRecette[$i]['id_ingredient']. '.png')) { // si le fichier existe déjà
                            $aIngredientRecette[$i]['thumb'] = 'public/img_ing/'.$aIngredientRecette[$i]['id_ingredient']. '.png'; 
                        }elseif(file_exists('public/img_ing/'.$aIngredientRecette[$i]['id_ingredient']. '.jpg')) {
                            $aIngredientRecette[$i]['thumb'] = 'public/img_ing/'.$aIngredientRecette[$i]['id_ingredient']. '.jpg'; 
                        }elseif(file_exists('public/img_ing/'.$aIngredientRecette[$i]['id_ingredient']. '.jpeg')) {
                            $aIngredientRecette[$i]['thumb'] = 'public/img_ing/'.$aIngredientRecette[$i]['id_ingredient']. '.jpeg'; 
                        }else{
                            $aIngredientRecette[$i]['thumb'] = 'public/img_ing/null.png';
                        }
                    }
                }
            }
