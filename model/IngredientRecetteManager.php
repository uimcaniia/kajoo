...
class IngredientRecetteManager extends Bdd
{
    ...
    public function add(IngredientRecette $ingredient_recette)
    {
        ...
        $param4 = $ingredient_recette->getAjout_recette();

        $request = 'INSERT INTO ' . self::TAB_INGREC . '(quantity, id_ingredient, id_recette, ajout_recette) VALUES (:quantity, :id_ingredient, :id_recette, :ajout_recette)';
        $arr = array(
            array(
                ":quantity",
                $param1
        ...
        $request2 = 'SELECT * FROM ' . self::TAB_INGREC . ' WHERE id_ingredient  = :id_ingredient AND id_recette = :id_recette AND ajout_recette = :ajout_recette';
        $arr2 = array(
            array(
                ":id_recette",
                $param3
...
/*    public function update($col, $val, $idcategory, $id_user)
    ...
    {
        $request = 'INSERT INTO ' . self::TAB_INGREC . ' (quantity, id_ingredient, id_recette, ajout_recette) SELECT quantity, id_ingredient, :id_newRecette, ajout_recette FROM ' . self::TAB_INGREC . ' where id_recette = :id_recette';
        $arr = array(
            array(
                ":id_recette",
                $id_recette
